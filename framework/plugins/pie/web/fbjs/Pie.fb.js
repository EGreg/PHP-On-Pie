	...
	me.ajaxExtend = function(what, slots, callback, options) {
		...
		if (typeof(what) == 'string') {
			...
			if (typeof(callback) != 'undefined') {
				what2 += '&' + escape('_pie[callback]') + '=' + escape(callback);
			}
			if (options && ('echo' in options)) {
				what2 += '&' + escape('_pie[echo]') + '=' + escape(options.echo);
			}
			if (options && ('post' in options)) {
				what2 += '&' + escape('_pie[post]') + '=1';
			}
			if ('nonce' in Pie) {
		...
		switch (me.typeOf(callables)) {
		 ...
		 case 'string':
			if (callables.substr(0, 7) == 'http://') {
				document.location = callables; // facebook does the rest
				return 0;
			}
